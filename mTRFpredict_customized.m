 function [pred,r,p,mse] = mTRFpredict_customized(stim,resp,model,fs,map,tmin,tmax,c)
...
% Generate lag matrix
X = [ones(size(x)),lagGen(x,tmin:tmax)]; %combine matrix of 1 and xLag
    %horizontally

% Calculate prediction
model = [c;reshape(model,size(model,1),size(model,2))];
pred = X*model;
