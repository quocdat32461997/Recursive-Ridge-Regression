function [A, b, w, M, t, tMin , tMax] = recursive_mTRFtrain(stim,resp,fs,map,tmin,tmax,lambda)
...
%   should be set in millis econds between TMIN and TMAX. Regularisation is
...
%below lines are commented out because stim and resp need to be in the same
%dimension
%tmin = floor(tmin/1e3*fs*map); %round down to the next integer
%tmax = ceil(tmax/1e3*fs*map); %round up to the next integer
...
dim = size(X,2);
if size(x,2)== 1
    d  = 2*eye(dim,dim);d([1,end]) = 1; %eye-geberate identity matrix
...
b = X'*y;
% Calculate model
model = inv((X'*X+lambda*M))*(X'*y);
w = reshape(model(size(x,2)+1:end,:),size(x,2),length(tmin:tmax),size(y,2));
t = (tmin:tmax)/fs*1e3;
tMin = tmin;
tMax = tmax;
end
